...
/**
 * @swagger
 * /api/v1/chapter/{id}/link:
 *   post:
 *     summary: Link a file to a chapter (move or copy)
 *     tags: [Volumes]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *     responses:
 *       200:
 *         description: File linked to chapter
 *       400:
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (existingFile) {
      // Update existing record to link to this chapter
    ...
    console.error('Error linking file to chapter:', error);
    return NextResponse.json(
